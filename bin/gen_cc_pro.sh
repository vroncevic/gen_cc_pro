...
function __gen_cc_pro() {
	...
	if [ -n "${PN}" ]; then
		...
		__load_conf "$GEN_CC_PRO_CFG" config_gen_cc_pro
		STATUS_CONF=$?
		declare -A config_gen_cc_pro_util=()
		__load_util_conf "$GEN_CC_PRO_UTIL_CFG" config_gen_cc_pro_util
		STATUS_CONF_UTIL=$?
		...
		local USRID=${config_gen_cc_pro_util[UID]}
		local GRPID=${config_gen_cc_pro_util[GID]}
